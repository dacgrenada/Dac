...
function generatePropertyLayout(isPaid = false) {
  ...
  // Watermark always in DOM — shown/hidden by body.locked CSS class
  const watermarkHtml = `
    <div class="watermark">
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ── Watermark — always in DOM, visible only when body.locked ── */
    .watermark {
      display: none;
      position: fixed;
      top: 0; left: 0;
    ...
    body.locked .watermark { display: flex; }
    .wm-text {
      font-family: 'Syne', sans-serif;
      font-size: 80px;
      font-weight: 800;
    ...
    @media print {
      ...
      .layout-card { border-color: #c87700 !important; max-width: 100% !important; width: 100% !important; page-break-inside: avoid; break-inside: avoid; }
      .satellite-wrap img, .sv-wrap img { max-height: 260px !important; }
      /* LOCKED: block all printing, force watermark visible */
      body.locked * { visibility: hidden !important; }
      body.locked .watermark { display: flex !important; visibility: visible !important; opacity: 0.65 !important; }
      body.locked .watermark * { visibility: visible !important; }
    }
...
<script>
  ...
  function unlockLayout() {
    // Single line — removes watermark AND re-enables printing simultaneously
    document.body.classList.remove('locked');
    // Hide waiting bar, show paid banner
...
document.addEventListener('click', (e) => {
  if (e.target.id === 'btn-layout' || e.target.closest('#btn-layout')) {
    // Check if already paid — open clean layout directly
    if (currentCode && isCodePaid(currentCode)) {
      generatePropertyLayout(true);
    } else {
      generatePropertyLayout(false); // Preview with watermark + pay button
    }
  }
});
