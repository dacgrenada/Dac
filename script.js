...
// Every print action requires a fresh payment.
...
// Fetch an external image URL and return a base64 data URL.
// This is required so images render in the print dialog (cross-origin <img> tags are blocked by browser print).
  ...
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      try {
        ...
        // Canvas tainted (CORS) – fall back to the original URL
        resolve(url);
      }
    };
    img.onerror = () => resolve('');
    // Cache-bust to maximise chance of CORS headers being honoured
...
function generatePropertyLayout(isPaid = false) {
  ...
  showToast('Generating preview… loading images');
  ...
  // ── Preload both map images as base64 BEFORE writing the popup ──
  // This guarantees the print dialog renders them immediately without waiting.
  const imageFetches = [fetchImageAsDataUrl(staticMapUrl)];
  if (svStaticUrl) imageFetches.push(fetchImageAsDataUrl(svStaticUrl));

  Promise.all(imageFetches).then(([satDataUrl, svDataUrl]) => {
    _openLayoutPopup({ isPaid, lat, lng, zoom, satDataUrl: satDataUrl || staticMapUrl, svDataUrl: svDataUrl || svStaticUrl, svStaticUrl, regionName, dateStr, qrDataUrl });
  });
}

function _openLayoutPopup({ isPaid, lat, lng, zoom, satDataUrl, svDataUrl, svStaticUrl, regionName, dateStr, qrDataUrl }) {
...
<div class="layout-card">
  ...
  <div class="satellite-wrap" id="sat-wrap">
    <div class="sat-img-loading" id="sat-loading" style="${satDataUrl && satDataUrl.startsWith('data:') ? 'display:none;' : ''}">
    ...
    <div class="sat-img-error" id="sat-error">
      ...
      <a href="${satDataUrl}" target="_blank" style="color:#f0a500;font-size:10px;margin-top:4px;display:inline-block;">Open image directly ↗</a>
    </div>
    <img
      class="sat-img"
      id="sat-img"
      ...
      style="${satDataUrl ? 'display:block;' : 'display:none;'}"
      onload="
  ...
  <div class="sv-wrap" id="sv-wrap">
    <div class="sv-img-loading" id="sv-img-loading" style="${svDataUrl ? 'display:none;' : ''}">
    ...
    <img
      class="sv-img"
      id="sv-img"
      src="${svDataUrl || svStaticUrl}"
      alt="Street View of ${currentCode}"
      style="${svDataUrl ? 'display:block;' : 'display:none;'}"
      onload="
...
<script>
  ...
  function checkPayment() {
    let paid = false;
    try {
      const confirmed = localStorage.getItem('dac_payment_confirmed');
      if (confirmed === DAC_CODE) {
        paid = true;
        // Clear immediately after reading — one-time signal, not a permanent unlock
        localStorage.removeItem('dac_payment_confirmed');
      }
    } catch(e) {}
    if (paid) {
  ...
  function unlockLayout() {
    ...
    printStyle.textContent = '@media print { body { background: #fff; padding: 0; } .layout-card { border-color: #c87700; } .no-print { display: none !important; } }';
...
document.addEventListener('click', (e) => {
  if (e.target.id === 'btn-layout' || e.target.closest('#btn-layout')) {
    // Always require payment — no permanent unlock allowed
