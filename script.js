...
const STRIPE_PAYMENT_LINK = 'https://buy.stripe.com/test_8x26oB05y81Z4cL7cR5wI04';
...
function generatePropertyLayout(isPaid = false) {
  ...
  // Watermark is ALWAYS in the DOM — shown/hidden by body.locked CSS class
  const watermarkHtml = `
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ── Watermark — always in DOM, shown only when body.locked ── */
    .watermark {
      display: none;
      position: fixed;
      top: 0; left: 0;
    ...
    .wm-text {
      ...
      white-space: nowrap;
      letter-spacing: 0.06em;
      text-shadow: 0 0 20px rgba(255,0,0,0.9), 2px 2px 0 #000, -2px -2px 0 #000;
    ...
    .wm-sub {
      ...
      white-space: nowrap;
      letter-spacing: 0.14em;
      text-shadow: 0 0 16px rgba(240,165,0,0.9), 2px 2px 0 #000;
    ...
    @media print {
      ...
      /* LOCKED: block all printing, show only watermark */
      body.locked * { visibility: hidden !important; }
      body.locked .watermark,
      body.locked .watermark * { visibility: visible !important; display: flex !important; opacity: 0.65 !important; }
    }
...
<script>
  ...
  function unlockLayout() {
    // Remove locked class — this hides watermark AND re-enables printing in one shot
