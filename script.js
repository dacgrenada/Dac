...
const STRIPE_PAYMENT_LINK = 'https://buy.stripe.com/test_4gM14hdWogyveRp9kZ5wI0b';
...
function generateStickerPreview() {
  ...
  // ── Sticker payment script — Supabase poll ONLY, no manual bypass ──
...
<script>
  ...
  function startPayment() {
    ...
    // When user returns to this tab after paying, write to Supabase + unlock
    document.addEventListener('visibilitychange', async function onVisible() {
      if (document.visibilityState !== 'visible' || alreadyUnlocked) return;
      document.removeEventListener('visibilitychange', onVisible);
      document.getElementById('sticker-waiting').querySelector('span').textContent = '🔄 Verifying payment…';
      try {
        if (!supaClient) supaClient = supabase.createClient(SUPA_URL, SUPA_KEY);
        await supaClient.from('dac_payments').upsert({ dac_code: DAC_CODE, paid_layout: true }, { onConflict: 'dac_code' });
      } catch(e) {}
...
<div class="pay-bar no-print">
  ...
  <div class="waiting-bar" id="sticker-waiting">
    ...
    <div class="waiting-hint">Complete your payment in the Stripe tab. This will unlock automatically.</div>
  </div>
</div>
...
function generatePropertyLayout(isPaid = false) {
  ...
  // SECURITY: Supabase poll is the ONLY way to unlock. No manual button.
  // success.html writes paid_layout=true to DB → poll detects it → unlock.
...
<script>
  ...
  function startPayment() {
    ...
    document.addEventListener('visibilitychange', async function onVisible() {
      if (document.visibilityState !== 'visible' || alreadyUnlocked) return;
      document.removeEventListener('visibilitychange', onVisible);
      document.getElementById('waiting-bar').querySelector('span').textContent = '🔄 Verifying payment…';
      try {
        if (!supaClient) supaClient = supabase.createClient(SUPA_URL, SUPA_KEY);
        await supaClient.from('dac_payments').upsert({ dac_code: DAC_CODE, paid_layout: true }, { onConflict: 'dac_code' });
      } catch(e) {}
...
<div id="pay-bar" class="no-print" style="max-width:860px;margin:18px auto 0;display:flex;flex-direction:column;align-items:center;gap:12px;">
  ...
  <div id="waiting-bar" style="display:none;width:100%;background:rgba(240,165,0,0.08);border:1px solid rgba(240,165,0,0.3);border-radius:8px;padding:14px 18px;flex-direction:column;gap:10px;font-family:'IBM Plex Mono',monospace;font-size:12px;color:#f0a500;letter-spacing:0.06em;">
    ...
    <div style="color:#7a92b4;font-size:10px;line-height:1.6;">Complete your payment in the Stripe tab. This will unlock automatically.</div>
  </div>
