...
function generatePropertyLayout(isPaid = false) {
  ...
  const staticMapUrl = `https://maps.googleapis.com/maps/api/staticmap?center=${lat},${lng}&zoom=${zoom}&size=${size}&maptype=satellite&markers=color:yellow%7Clabel:P%7C${lat},${lng}&key=${MAPS_API_KEY}`;
  ...
  const watermarkHtml = !isPaid ? `
    <div class="watermark">
      <div class="wm-text">DAC – PREVIEW ONLY</div>
      <div class="wm-sub">NOT PAID VERSION</div>
    </div>` : '';
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @media print {
      ...
      ${!isPaid ? `.watermark { opacity: 0.55 !important; }` : ''}
    }
    ${!isPaid ? `
    /* Prevent actual printing of preview via JavaScript override */
    ` : ''}
...
<div class="layout-card">
  ...
  <div class="satellite-wrap">
    <img class="sat-img" src="${staticMapUrl}" alt="Satellite view" />
    <div class="sat-label">📡 SATELLITE VIEW</div>
  </div>
...
${!isPaid ? `
<button class="print-btn no-print" onclick="alert('⚠️ Payment required to print the clean address layout.\\n\\nClick \\'Pay to Print Clean Address Layout\\' on the main DAC page to complete payment and unlock this feature.')" style="background:#555;cursor:not-allowed;opacity:0.7;" title="Payment required">
