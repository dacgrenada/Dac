...
function generatePropertyLayout(isPaid = false) {
  ...
  if (isPaid && !isCodePaid(currentCode)) {
    showToast('Payment required to unlock clean layout');
    return;
  }
  ...
  const watermarkHtml = !isPaid ? `
    ...
    <div class="print-blocked-msg" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:#0c1220;z-index:99999;align-items:center;justify-content:center;flex-direction:column;gap:16px;font-family:'IBM Plex Mono',monospace;text-align:center;padding:40px;">
      <div style="font-size:48px;">🔒</div>
      <div style="font-size:22px;font-weight:800;color:#f0a500;font-family:'Syne',sans-serif;">PRINT LOCKED</div>
      <div style="font-size:13px;color:#e8eef6;">Payment required to print the clean layout.</div>
      <div style="font-size:11px;color:#7a92b4;margin-top:4px;">Return to DAC and click <strong style="color:#f0a500;">Pay to Print Layout</strong></div>
    </div>` : '';
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .satellite-wrap img.sat-img {
      width: 100%;
      display: block;
      /* No max-height / object-fit crop — show full image at natural ratio */
      object-fit: fill;
    }
    .sat-img-loading {
      display: flex;
    ...
    @media print {
      ...
      .layout-card { border-color: #c87700; }
      .no-print { display: none !important; }
      ${!isPaid ? `
      /* Force watermark visible and prominent when printing preview */
      .watermark { opacity: 0.35 !important; display: flex !important; }
      /* Block all content from printing on preview */
      body * { visibility: hidden !important; }
      .watermark, .watermark * { visibility: visible !important; }
      .print-blocked-msg { visibility: visible !important; display: block !important; }
      ` : ''}
    }
...
${!isPaid ? `
<button class="print-btn no-print" style="background:#3a3a3a;cursor:not-allowed;opacity:0.7;" title="Payment required" onclick="showPrintBlocked()">
  🔒 Print Locked – Payment Required
</button>
<div class="no-print" style="text-align:center;margin-top:10px;font-family:'IBM Plex Mono',monospace;font-size:12px;color:#e85d3a;background:rgba(232,93,58,0.1);border:1px solid rgba(232,93,58,0.4);border-radius:6px;padding:8px 16px;max-width:860px;margin:12px auto 0;">
  ⚠️ Payment required to print or download the clean address layout
</div>
<script>
  function showPrintBlocked() {
    const msg = document.querySelector('.print-blocked-msg');
    if (msg) { msg.style.display = 'flex'; setTimeout(() => { msg.style.display = 'none'; }, 3500); }
    alert('🔒 Print Locked\\n\\nPayment is required to print or save this layout.\\n\\nClose this window, return to DAC and click \\"Pay to Print Layout\\" to unlock.');
  }
  // Block Ctrl+P / Cmd+P
  ...
  // Block File → Print menu
  window.addEventListener('beforeprint', function() {
    showPrintBlocked();
  });
  // Override window.print()
  window.print = function() { showPrintBlocked(); };
<\/script>` : `
...
document.addEventListener('click', (e) => {
  if (e.target.id === 'btn-layout' || e.target.closest('#btn-layout')) {
    generatePropertyLayout(false); // Always preview with watermark
  }
});
...
document.addEventListener('click', (e) => {
  if (e.target.id === 'btn-paid-layout' || e.target.closest('#btn-paid-layout')) {
    ...
    // Check if this code has already been paid for
    if (isCodePaid(currentCode)) {
      generatePropertyLayout(true);
      return;
    }

    // Save current code so success.html can retrieve it
    saveCurrentCodeForPayment(currentCode);

    // Build return URL with the DAC code as a query param so success page knows which code to unlock
    const successUrl = SUCCESS_URL + '?code=' + encodeURIComponent(currentCode) +
      ...
      (streetViewStaticUrl ? '&sv=' + encodeURIComponent(streetViewStaticUrl) : '');
