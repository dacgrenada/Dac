...
const STRIPE_PAYMENT_LINK = 'https://buy.stripe.com/test_8x24gtcSkaa77oX68N5wI0d';
...
function generateStickerPreview() {
  ...
  // ── Sticker payment script — waits for BroadcastChannel signal from success.html ──
  const stickerPayScript = `
<script>
  const STRIPE_LINK    = '${STRIPE_PAYMENT_LINK}';
  const EXPECTED_CODE  = '${currentCode}';
  let paymentStarted   = false;
  let unlocked         = false;

  // Listen for confirmed payment signal BEFORE user clicks pay,
  // so the channel is open and ready when success.html posts to it.
  var bc = null;
  try {
    ...
    bc.onmessage = function(evt) {
      if (unlocked) return;
      var data = evt.data || {};
      if (data.paid && data.code === EXPECTED_CODE) {
        unlocked = true;
  ...
  window.addEventListener('storage', function(e) {
    if (unlocked) return;
    if (e.key === 'dac_sticker_paid_' + EXPECTED_CODE && e.newValue === '1') {
      unlocked = true;
      unlockSticker();
    }
  });
  ...
  function startPayment() {
    ...
    paymentStarted = true;
    // Store the pending code so success.html can read it
    try { localStorage.setItem('dac_pending_payment_code', EXPECTED_CODE); } catch(e) {}
...
<head>
  ...
  <!-- Blocks printing until paid — removed by unlockSticker() -->
...
function generatePropertyLayout(isPaid = false) {
  ...
  // ── Layout payment script — waits for BroadcastChannel signal from success.html ──
...
<script>
  ...
  window.addEventListener('storage', function(e) {
    if (unlocked) return;
    if (e.key === 'dac_paid_' + EXPECTED_CODE && e.newValue === '1') {
      unlocked = true;
      unlockLayout();
    }
  });
