...
function handleMapClick(e) {
  ...
  // ✅ SUPABASE — save on EVERY pin drop (not just after payment)
...
function generatePropertyLayout(isPaid = false) {
  ...
  // These are self-contained: they bring their own Supabase client and
  // poll localStorage (fast) with a Supabase fallback (cross-tab reliable).
...
<script>
  ...
  function getSupaClient() {
    if (!supaClient && typeof supabase !== 'undefined') {
  ...
  function startPayment() {
    ...
    // ── Poll 2: Supabase (cross-tab fallback, every 3s) ───────────────
    supaInterval = setInterval(async () => {
      ...
      try {
        ...
        const { data } = await client
          ...
          .single();
        if (data && data.paid_layout === true) {
          clearInterval(pollInterval);
          clearInterval(supaInterval);
          // Also set localStorage so the fast poll won't keep firing
