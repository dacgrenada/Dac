...
async function supabaseSaveCode(code, lat, lng, parish){
  // Do NOT save before payment
  localStorage.setItem("dac_pending_code", code);
  localStorage.setItem("dac_lat", lat);
  localStorage.setItem("dac_lng", lng);
  localStorage.setItem("dac_parish", parish);
  console.log("Stored locally until payment:",code);
}
...
// Produces CC-PPP-XXXXXX  e.g. GD-STG-A7F3K9
...
function detectRegion(lat, lng, countryCode) {
  ...
  // All other countries: use country code as 3-char region
...
function handleMapClick(e) {
  ...
  // ✅ SUPABASE — save on every pin drop
  supabaseSaveCode(code, lat, lng, regionName);
}
...
// Shared print CSS injected both at page load (isPaid) and after payment unlock
...
function generatePropertyLayout(isPaid = false) {
  ...
  // scale=2 → renders 1280x800 — crisp for print
...
<script>
  ...
  // Immediately wipe any stale paid flag so going back without paying never unlocks
  try { localStorage.removeItem(PAID_KEY); } catch(e) {}
  try { localStorage.removeItem('dac_pending_payment_code'); } catch(e) {}

  function startPayment() {
    if (paymentStarted) return; // prevent double-click
    paymentStarted = true;

    // Store pending code so success.html can pick it up
    try { localStorage.setItem('dac_pending_payment_code', DAC_CODE); } catch(e) {}

    // Open Stripe in new tab; this preview page stays open
    ...
    // Only start polling AFTER Pay is clicked
    pollInterval = setInterval(checkPayment, 2000);
  }

  function checkPayment() {
    // Only accept the key if payment was actually started from this window
    ...
    if (paid) {
      ...
      unlockLayout();
      // ✅ SUPABASE — mark paid_layout = true
      supabaseMarkPaid(DAC_CODE);
    }
  }
  ...
  function unlockLayout() {
    ...
    // Inject clean paid print styles
...
document.addEventListener('click', (e) => {
  if (e.target.id === 'btn-layout' || e.target.closest('#btn-layout')) {
    // Always require payment — never bypass for previously-paid codes
    generatePropertyLayout(false);
  }
});


// Payment unlock listener
function dacCheckPayment(code){
 const key="dac_paid_"+code;
 const t=setInterval(()=>{
   if(localStorage.getItem(key)){
     console.log("Payment confirmed – unlocking layout");
     const btn=document.getElementById("btn-print");
     if(btn) btn.disabled=false;
     const wm=document.querySelector(".preview-watermark");
