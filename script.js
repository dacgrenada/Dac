...
// ✅ Called on EVERY pin drop — saves code + coordinates to Supabase immediately
async function supabaseSaveCode(code, lat, lng, parish) {
  try {
    const { error } = await db.from('dac_payments').upsert({
      dac_code: code, latitude: lat, longitude: lng,
      parish: parish, paid_layout: false
    }, { onConflict: 'dac_code' });
    if (error) console.warn('Supabase upsert warning:', error.message);
    else console.log('%c✅ Supabase saved: ' + code, 'color:#3abf9e;font-weight:bold;');
  } catch (err) { console.error('Supabase save error:', err); }
}

async function supabaseSearchCode(code) {
  try {
    const { data, error } = await db.from('dac_payments').select('*').eq('dac_code', code).single();
    if (error || !data) return null;
    return data;
  } catch (err) { console.error('Supabase search error:', err); return null; }
}

async function supabaseMarkPaid(code) {
  try {
    const { error } = await db.from('dac_payments').update({ paid_layout: true }).eq('dac_code', code);
    if (error) console.warn('Supabase paid update:', error.message);
    else console.log('%c✅ Supabase paid_layout=true: ' + code, 'color:#f0a500;font-weight:bold;');
  } catch (err) { console.error('Supabase paid error:', err); }
}
...
const STRIPE_PAYMENT_LINK = 'https://buy.stripe.com/test_00wfZb6tWeqnfVt2WB5wI08';
...
function generateStickerPreview() {
  ...
  // ── Sticker payment script (identical unlock pattern to layout) ──
  const stickerPayScript = `
<script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"><\/script>
<script>
  const DAC_CODE    = '${currentCode.replace(/'/g, "\\'")}';
  const PAID_KEY    = 'dac_sticker_paid_' + DAC_CODE;
  const STRIPE_LINK = '${STRIPE_PAYMENT_LINK}';
  const SUPA_URL    = '${SUPABASE_URL}';
  const SUPA_KEY    = '${SUPABASE_KEY}';

  let supaClient      = null;
  let supaInterval    = null;
  let paymentStarted  = false;
  let alreadyUnlocked = false;

  // Layer 1 — BroadcastChannel (instant, blob: URL safe)
  var bc = null;
  try {
    bc = new BroadcastChannel('dac_sticker_payment');
    bc.onmessage = function(e) {
      if (e.data && e.data.code === DAC_CODE && e.data.paid === true) triggerUnlock();
    };
  } catch(e) {}

  // Layer 2 — storage event
  window.addEventListener('storage', function(e) {
    if (e.key === PAID_KEY && e.newValue === '1') triggerUnlock();
  });

  function startPayment() {
    if (paymentStarted) return;
    paymentStarted = true;
    try { localStorage.setItem('dac_sticker_pending', DAC_CODE); } catch(e) {}
    window.open(STRIPE_LINK, '_blank');
    document.getElementById('btn-sticker-pay').style.display = 'none';
    document.getElementById('sticker-waiting').style.display = 'flex';

    // Layer 3 — Supabase poll every 3s
    try { supaClient = supabase.createClient(SUPA_URL, SUPA_KEY); } catch(e) {}
    supaInterval = setInterval(async function() {
      if (alreadyUnlocked) { clearInterval(supaInterval); return; }
      if (!supaClient) return;
      try {
        var res = await supaClient
          .from('dac_payments')
          .select('paid_sticker')
          .eq('dac_code', DAC_CODE)
          .single();
        if (res.data && res.data.paid_sticker === true) triggerUnlock();
      } catch(e) {}
    }, 3000);
  }
  ...
  async function manualVerify() {
    ...
    document.getElementById('sticker-waiting-msg').textContent = '🔄 Verifying with Supabase…';
    try {
      if (!supaClient) supaClient = supabase.createClient(SUPA_URL, SUPA_KEY);
      await supaClient
        .from('dac_payments')
        .upsert({ dac_code: DAC_CODE, paid_sticker: true }, { onConflict: 'dac_code' });
      triggerUnlock();
    } catch(e) {
      triggerUnlock();
    }
  ...
  function unlockSticker() {
    // Remove watermark
    var wm = document.getElementById('sticker-wm');
    if (wm) wm.remove();
    // Remove print block
    var pb = document.getElementById('print-block');
    if (pb) pb.remove();
    // Inject clean print styles
    var s = document.createElement('style');
    s.textContent = '@media print { .no-print { display:none!important; } body { background:#fff!important; } }';
    document.head.appendChild(s);
    // Show success bar
    document.getElementById('sticker-waiting').style.display = 'none';
    document.getElementById('sticker-unlocked').style.display = 'block';
  }
<\/script>`;
...
<head>
  <meta charset="UTF-8"/>
  <title>DAC Sticker – ${currentCode}</title>
  ...
  <style>
    * { box-sizing:border-box; margin:0; padding:0; }
    body { background:#0c1220; font-family:'DM Sans',sans-serif; min-height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:flex-start; padding:32px 16px; gap:20px; }
    @keyframes spin { to { transform:rotate(360deg); } }
    @keyframes wm-pulse { 0%,100%{opacity:0.55} 50%{opacity:0.7} }
    ...
    .sticker-flag  { font-size:2.8rem; line-height:1; margin-bottom:4px; }
    .sticker-logo  { font-family:'Syne',sans-serif; font-size:2rem; font-weight:800; color:#0c1220; letter-spacing:0.06em; line-height:1; }
    .sticker-sub   { font-family:'IBM Plex Mono',monospace; font-size:0.6rem; text-transform:uppercase; letter-spacing:0.14em; color:#7a92b4; margin-top:3px; }
    .sticker-code  { font-family:'IBM Plex Mono',monospace; font-size:1.35rem; font-weight:700; color:#c87700; letter-spacing:0.08em; margin:14px 0 10px; word-break:break-all; }
    .sticker-qr    { width:160px; height:160px; margin:0 auto 12px; display:block; border:2px solid #f0a500; border-radius:8px; padding:6px; background:#fff; }
    .sticker-coords{ font-family:'IBM Plex Mono',monospace; font-size:0.65rem; color:#555; margin-top:4px; }
    .sticker-scan  { font-family:'IBM Plex Mono',monospace; font-size:0.65rem; color:#888; margin-top:2px; text-transform:uppercase; letter-spacing:0.1em; }
    .sticker-date  { font-family:'IBM Plex Mono',monospace; font-size:0.6rem; color:#aaa; margin-top:8px; border-top:1px dashed #eee; padding-top:8px; }
    ...
    .wm-text { font-family:'Syne',sans-serif; font-size:2.8rem; font-weight:800; color:#ff0000; transform:rotate(-25deg); white-space:nowrap; letter-spacing:0.06em; text-shadow:2px 2px 0 #000,-2px -2px 0 #000; -webkit-text-stroke:1.5px #000; line-height:1; }
    .wm-sub  { font-family:'IBM Plex Mono',monospace; font-size:1rem; font-weight:700; color:#f0a500; transform:rotate(-25deg); white-space:nowrap; letter-spacing:0.14em; text-shadow:1px 1px 0 #000,-1px -1px 0 #000; margin-top:4px; }

    /* ── Pay bar ── */
    .pay-bar { width:100%; max-width:320px; display:flex; flex-direction:column; gap:10px; }
    .btn-pay {
      ...
      box-shadow:0 0 20px rgba(155,114,232,0.25);
      display:flex; align-items:center; justify-content:center; gap:10px;
    }
    .btn-badge { font-family:'IBM Plex Mono',monospace; font-size:10px; background:#9b72e8; color:#fff; padding:2px 8px; border-radius:4px; letter-spacing:0.08em; }
    ...
    .waiting-bar {
      ...
      font-family:'IBM Plex Mono',monospace; font-size:11px; color:#f0a500; letter-spacing:0.06em;
    }
    .waiting-row { display:flex; align-items:center; gap:10px; }
    .waiting-hint { color:#7a92b4; font-size:10px; line-height:1.6; }
    .btn-manual {
      width:100%; padding:9px 14px;
      background:linear-gradient(135deg,#1a3a2a,#1e2d44);
      color:#3abf9e; border:1px solid #3abf9e; border-radius:8px;
    ...
    .unlocked-bar {
      ...
      background:rgba(58,191,158,0.12); border:2px solid #3abf9e;
      border-radius:10px; padding:16px; text-align:center;
    }
    .unlocked-title { font-family:'Syne',sans-serif; font-size:18px; font-weight:800; color:#3abf9e; margin-bottom:8px; }
    .btn-print-now {
      padding:10px 28px;
      background:#3abf9e; color:#0c1220;
    ...
    <style id="print-block">
      @media print {
        ...
        .sticker-watermark, .sticker-watermark * { visibility:visible!important; }
      }
    </style>
  </style>
</head>
...
<div class="sticker-wrap">
  ...
  <!-- Watermark overlay -->
  <div class="sticker-watermark" id="sticker-wm">
    <div class="wm-text">PREVIEW</div>
    <div class="wm-sub">PAY TO PRINT</div>
  </div>

  <div class="sticker-flag">🇬🇩</div>
  <div class="sticker-logo">DAC</div>
  <div class="sticker-sub">Digital Address Codes</div>
  <div class="sticker-code">${currentCode}</div>
  ${qrUrl
    ? `<img class="sticker-qr" src="${qrUrl}" alt="QR Code"/>`
    : `<div class="sticker-qr" style="display:flex;align-items:center;justify-content:center;font-size:10px;color:#aaa;">QR unavailable</div>`
  }
  <div class="sticker-coords">${lat}, ${lng}</div>
  <div class="sticker-scan">📱 Scan for directions</div>
  <div class="sticker-date">Generated ${dateStr}</div>
</div>

<!-- Pay bar -->
<div class="pay-bar no-print">
  <button class="btn-pay" id="btn-sticker-pay" onclick="startPayment()">
    💳 Pay to Print Sticker — $9
    <span class="btn-badge">NO WATERMARK</span>
  </button>

  <div class="waiting-bar" id="sticker-waiting">
    <div class="waiting-row">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#f0a500" stroke-width="2" style="animation:spin 1s linear infinite;flex-shrink:0"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg>
      <span id="sticker-waiting-msg">⏳ Complete payment in the Stripe tab…</span>
    </div>
    <div class="waiting-hint">After paying, Stripe shows "Thanks for your payment".<br>Then click below to unlock your sticker.</div>
    <button class="btn-manual" id="btn-manual-verify" onclick="manualVerify()">✅ I've Paid — Unlock My Sticker</button>
  </div>
</div>

<div class="unlocked-bar no-print" id="sticker-unlocked">
  <div class="unlocked-title">✅ Sticker Unlocked!</div>
  <button class="btn-print-now" onclick="window.print()">🖨 Print Sticker</button>
</div>
...
function generatePropertyLayout(isPaid = false) {
  ...
  // ── Inline payment scripts for the popup window ──────────────────────────
  // The popup is opened as a blob: URL so it shares the same origin as the
  // parent page — enabling localStorage, storage events AND BroadcastChannel.
  // Three-layer unlock: BroadcastChannel (instant) → storage event → Supabase poll.
  const paymentScript = !isPaid ? `
<script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"><\/script>
<script>
  const DAC_CODE    = '${currentCode.replace(/'/g, "\\'")}';
  ...
  // ── Layer 1: BroadcastChannel — instant, same-origin, works with blob URLs ──
  var bc = null;
  try {
    bc = new BroadcastChannel('dac_payment');
    bc.onmessage = function(e) {
      if (e.data && e.data.code === DAC_CODE && e.data.paid === true) {
        triggerUnlock();
      }
    };
  } catch(e) {}

  // ── Layer 2: storage event — fires when another same-origin tab sets localStorage ──
  window.addEventListener('storage', function(e) {
    if (e.key === PAID_KEY && e.newValue === '1') {
      triggerUnlock();
    }
  });
  ...
  function startPayment() {
    ...
    // Store code so success.html can read it back
    ...
    document.getElementById('waiting-bar').style.display = 'flex';

    // ── Layer 3: Poll Supabase every 3s — works regardless of origin ──
    try { supaClient = supabase.createClient(SUPA_URL, SUPA_KEY); } catch(e) {}
    ...
    supaInterval = setInterval(async function() {
      ...
      try {
        var res = await supaClient
          ...
          .eq('dac_code', DAC_CODE)
          .single();
        if (res.data && res.data.paid_layout === true) {
          triggerUnlock();
        }
      } catch(e) {}
    }, 3000);
  ...
  // Called when user clicks "I've Paid" button — queries Supabase directly
  async function manualVerify() {
    var btn = document.getElementById('btn-verify');
    if (btn) { btn.textContent = '🔄 Checking…'; btn.disabled = true; }
    document.getElementById('waiting-msg').textContent = '🔄 Verifying payment with Supabase…';
    ...
    try {
      ...
      if (res.data && res.data.paid_layout === true) {
        ...
        // Not confirmed in DB yet — could be Stripe webhook delay
        // Save it manually and unlock anyway since user confirmed payment
        await supaClient
          .from('dac_payments')
          .upsert({ dac_code: DAC_CODE, paid_layout: true }, { onConflict: 'dac_code' });
        triggerUnlock();
      }
    } catch(e) {
      // Network error — unlock anyway, user confirmed payment visually
      triggerUnlock();
    }
  }
  ...
  function unlockLayout() {
    ...
    printStyle.textContent = \`
      ...
      @media print {
        ...
        body { display: block !important; color: #000 !important; }
        .no-print, #pay-bar { display: none !important; }
        .layout-card { border: 1.5px solid #c87700 !important; width: 186mm !important; max-width: 186mm !important; margin: 0 auto !important; background: #fff !important; border-radius: 6px !important; box-shadow: none !important; overflow: hidden !important; }
        .layout-header { background: #1a1a2e !important; border-color: #c87700 !important; padding: 8px 16px !important; }
        .header-left .logo { color: #c87700 !important; font-size: 22px !important; }
        .header-left .logo-sub { color: #aaa !important; }
        .header-right .code { color: #fff !important; font-size: 15px !important; }
...
<\/script>` : '';
  ...
  // Build HTML as a string then open as a Blob URL.
  // Blob URLs inherit the page's origin so localStorage, storage events
  // and BroadcastChannel all work correctly with the parent window.
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .header-left .logo {
      ...
      color: #f0a500; letter-spacing: 0.06em; line-height: 1;
    }
    .header-left .logo-sub {
      font-family: 'IBM Plex Mono', monospace;
      font-size: 9px; color: #7a92b4;
    ...
    .satellite-wrap {
      ...
      background: #1e2d44; border-bottom: 1px solid #243450;
      line-height: 0; min-height: 180px;
    }
    ...
    .sat-img-error {
      ...
      min-height: 180px; background: #1a2a3a; color: #e85d3a;
      font-family: 'IBM Plex Mono', monospace;
    ...
    .sat-label {
      ...
      font-family: 'IBM Plex Mono', monospace;
      font-size: 10px; color: #f0a500;
      padding: 3px 8px; border-radius: 4px;
      border: 1px solid rgba(240,165,0,0.4); letter-spacing: 0.08em;
    }
    .sat-coords {
      position: absolute; bottom: 10px; right: 12px;
      background: rgba(12,18,32,0.82);
      font-family: 'IBM Plex Mono', monospace;
      font-size: 9px; color: #e8eef6;
      padding: 3px 8px; border-radius: 4px;
      border: 1px solid rgba(255,255,255,0.1); letter-spacing: 0.06em;
    }

    .sv-wrap {
      position: relative; width: 100%;
      background: #1e2d44; border-bottom: 1px solid #243450;
      line-height: 0; min-height: 80px;
    }
    .sv-wrap img.sv-img {
      width: 100%; display: block;
      max-height: 200px; object-fit: fill;
    }
    .sv-img-loading {
      display: flex; align-items: center; justify-content: center;
      min-height: 120px; color: #7a92b4;
      font-family: 'IBM Plex Mono', monospace;
      font-size: 11px; gap: 10px; letter-spacing: 0.08em;
    }
    .sv-label-overlay {
      position: absolute; top: 10px; left: 12px;
      background: rgba(12,18,32,0.82);
      font-family: 'IBM Plex Mono', monospace;
      font-size: 10px; color: #3abf9e;
      padding: 3px 8px; border-radius: 4px;
      border: 1px solid rgba(58,191,158,0.4); letter-spacing: 0.08em;
    }

    .info-row { display: flex; align-items: stretch; border-top: 1px solid #243450; }
    .info-fields {
    ...
    .field-label {
      ...
      letter-spacing: 0.12em; color: #7a92b4; margin-bottom: 3px;
    }
    .field-value {
      font-family: 'IBM Plex Mono', monospace;
    ...
    .qr-panel {
      padding: 14px 20px; border-left: 1px solid #243450;
    ...
    .qr-panel img {
      ...
      border: 3px solid #f0a500; border-radius: 6px;
      padding: 4px; background: #fff; display: block;
    }
    .qr-panel .qr-label {
      font-family: 'IBM Plex Mono', monospace;
      font-size: 8px; color: #7a92b4;
      text-transform: uppercase; letter-spacing: 0.1em; text-align: center;
    }

    .layout-footer {
      background: #0c1220; border-top: 1px solid #243450;
      padding: 8px 24px;
      display: flex; justify-content: space-between; align-items: center;
    }
    .layout-footer span {
      font-family: 'IBM Plex Mono', monospace;
      font-size: 9px; color: #7a92b4; letter-spacing: 0.08em;
    }
    ...
    .wm-text {
      font-family: 'Syne', sans-serif;
      font-size: 80px; font-weight: 800; color: #ff0000;
      white-space: nowrap; letter-spacing: 0.06em;
      text-shadow: 0 0 20px rgba(255,0,0,0.9), 2px 2px 0 #000, -2px -2px 0 #000, 2px -2px 0 #000, -2px 2px 0 #000;
      -webkit-text-stroke: 2px #000;
    }
    .wm-sub {
      font-family: 'IBM Plex Mono', monospace;
      font-size: 34px; font-weight: 700; color: #f0a500;
      white-space: nowrap; letter-spacing: 0.14em;
      text-shadow: 0 0 16px rgba(240,165,0,0.9), 2px 2px 0 #000, -2px -2px 0 #000;
      -webkit-text-stroke: 1px #000;
    }
    @media print { .watermark { position: fixed !important; opacity: 0.65 !important; } }

    .print-btn {
      display: block; margin: 18px auto 0;
      padding: 10px 32px;
      background: #f0a500; color: #0c1220;
      font-family: 'Syne', sans-serif;
  ...
  ${!isPaid ? `<style id="print-block-style">
    ...
    @media print {
      ...
      .watermark { opacity: 0.65 !important; }
    }
  </style>` : ''}
</head>
<body>
...
<div class="layout-card">
  ...
  <div class="satellite-wrap" id="sat-wrap">
    ...
    <img
      class="sat-img" id="sat-img"
      src="${staticMapUrl}"
      alt="Satellite view"
      style="display:none;"
      onload="document.getElementById('sat-loading').style.display='none';document.getElementById('sat-error').style.display='none';this.style.display='block';"
      onerror="document.getElementById('sat-loading').style.display='none';document.getElementById('sat-error').style.display='flex';this.style.display='none';"
    />
  ...
  <div class="sv-wrap" id="sv-wrap">
    ...
    <img
      class="sv-img" id="sv-img"
      src="${svStaticUrl}"
      alt="Street View"
      style="display:none;"
      onload="document.getElementById('sv-img-loading').style.display='none';this.style.display='block';"
      onerror="document.getElementById('sv-img-loading').style.display='none';document.getElementById('sv-wrap').style.display='none';"
    />
...
<div id="pay-bar" class="no-print" style="max-width:860px;margin:18px auto 0;display:flex;flex-direction:column;align-items:center;gap:12px;">
  ...
  <div id="waiting-bar" style="display:none;width:100%;background:rgba(240,165,0,0.08);border:1px solid rgba(240,165,0,0.3);border-radius:8px;padding:14px 18px;flex-direction:column;gap:10px;font-family:'IBM Plex Mono',monospace;font-size:12px;color:#f0a500;letter-spacing:0.06em;">
    <div style="display:flex;align-items:center;gap:10px;">
      ...
      <span id="waiting-msg">⏳ Complete payment in the Stripe tab…</span>
    </div>
    <div style="color:#7a92b4;font-size:10px;line-height:1.6;">
      After paying, Stripe will show "Thanks for your payment".<br>
      Then click the button below to unlock your layout.
    </div>
    <button id="btn-verify" onclick="manualVerify()" style="
      width:100%;padding:10px 16px;margin-top:2px;
      background:linear-gradient(135deg,#1a3a2a,#1e2d44);
      color:#3abf9e;border:1px solid #3abf9e;border-radius:8px;
      font-family:'Syne',sans-serif;font-size:14px;font-weight:700;
      cursor:pointer;letter-spacing:0.03em;transition:all 0.2s;
    ">✅ I've Paid — Unlock My Layout</button>
  </div>
...
` : `<button class="print-btn no-print" onclick="window.print()">🖨 Print / Save as PDF</button>`}
  ...
  const blobUrl = URL.createObjectURL(blob);
  window.open(blobUrl, '_blank');
  // Revoke after a short delay — browser has already started loading it
  setTimeout(() => URL.revokeObjectURL(blobUrl), 10000);
}
