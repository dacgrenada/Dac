...
function initMap() {
  ...
  // Initialize Street View panorama in the side panel
...
let streetView   = null;
let svService    = null;
let currentView  = 'map'; // 'map' | 'split' | 'street'
...
function loadStreetView(lat, lng) {
  ...
  const noCoverage   = document.getElementById('sv-no-coverage');
  const addressBar   = document.getElementById('sv-address-bar');
  const mapViews     = document.getElementById('map-views');

  // Hide placeholder while searching
  if (placeholder) placeholder.style.display = 'none';
  if (noCoverage)  noCoverage.style.display  = 'none';

  svService.getPanorama(
    { location: { lat, lng }, radius: 100, preference: google.maps.StreetViewPreference.NEAREST },
    function(data, status) {
      if (status === google.maps.StreetViewStatus.OK) {
        // Coverage found — show Street View
        streetView.setPosition({ lat, lng });
        streetView.setPov({ heading: 0, pitch: 0 });

        // Auto-switch to split view on first pin
        if (currentView === 'map') setView('split');

        // Update address bar
        if (addressBar && data.location && data.location.description) {
          addressBar.textContent = '📍 ' + data.location.description;
        } else if (addressBar) {
          addressBar.textContent = `📍 ${lat.toFixed(5)}, ${lng.toFixed(5)}`;
        }

        showToast('Street View loaded');
      } else {
        // No coverage
        if (noCoverage)  { noCoverage.style.display  = 'flex'; }
        if (placeholder) { placeholder.style.display = 'none'; }
        if (addressBar)  { addressBar.textContent     = '⚠ No Street View coverage here'; }
        showToast('No Street View coverage at this location');
      }
    }
  );
}
...
function setView(mode) {
  ...
  const svPanel   = document.getElementById('street-view-panel');

  // Reset tab states
  [tabMap, tabSplit, tabStreet].forEach(t => t && t.classList.remove('active'));

  if (mode === 'map') {
    mapViews.style.gridTemplateColumns = '1fr 0px';
    if (svPanel) svPanel.style.display = 'none';
    if (tabMap) tabMap.classList.add('active');
  } else if (mode === 'split') {
    mapViews.style.gridTemplateColumns = '1fr 1fr';
    if (svPanel) svPanel.style.display = 'block';
    if (tabSplit) tabSplit.classList.add('active');
    // Trigger resize so map redraws correctly
    setTimeout(() => google.maps.event.trigger(map, 'resize'), 300);
  } else if (mode === 'street') {
    mapViews.style.gridTemplateColumns = '0px 1fr';
    if (svPanel) svPanel.style.display = 'block';
    if (tabStreet) tabStreet.classList.add('active');
  }
}

// Expose setView globally for onclick handlers in HTML
...
function handleMapClick(e) {
  ...
  // Load Street View automatically for pinned location
...
function updateUI(data) {
  ...
  ['btn-download','btn-print','btn-share','btn-layout','btn-paid-layout'].forEach(id => {
    const b = document.getElementById(id);
    if (b) { b.disabled = false; b.style.display = ''; }
  });

  // Update paid layout button to show unlocked state if already paid
  const paidBtn = document.getElementById('btn-paid-layout');
  if (paidBtn && isCodePaid(data.code)) {
    paidBtn.style.borderColor = '#3abf9e';
    paidBtn.style.color = '#3abf9e';
    const badge = paidBtn.querySelector('.btn-badge-paid');
    if (badge) { badge.textContent = 'UNLOCKED ✓'; badge.style.background = '#3abf9e'; badge.style.color = '#0c1220'; }
  } else if (paidBtn) {
    paidBtn.style.borderColor = '';
    paidBtn.style.color = '';
    const badge = paidBtn.querySelector('.btn-badge-paid');
    if (badge) { badge.textContent = 'NO WATERMARK'; badge.style.background = ''; badge.style.color = ''; }
  }
...
// ============================================================
// PROPERTY LAYOUT GENERATOR  — seamless payment flow
// ============================================================
...
function generatePropertyLayout() {
  ...
  // scale=2 → 1280×800 actual pixels — crisp for print
  ...
  win.document.write('<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/>'
    ...
    + 'body { background: #0c1220; color: #e8eef6; font-family: "DM Sans", sans-serif; padding: 24px; min-height: 100vh; }'
    + '.layout-card { background: #141e30; border: 2px solid #f0a500; border-radius: 12px; overflow: hidden; max-width: 860px; margin: 0 auto; position: relative; }'
    + '.layout-header { background: #0c1220; border-bottom: 2px solid #f0a500; padding: 14px 24px; display: flex; align-items: center; justify-content: space-between; }'
    + '.header-left .logo { font-family: "Syne", sans-serif; font-size: 28px; font-weight: 800; color: #f0a500; letter-spacing: 0.06em; }'
    + '.header-left .logo-sub { font-family: "IBM Plex Mono", monospace; font-size: 9px; color: #7a92b4; text-transform: uppercase; letter-spacing: 0.14em; margin-top: 3px; }'
    + '.header-right { text-align: right; }'
    + '.header-right .code { font-family: "IBM Plex Mono", monospace; font-size: 20px; font-weight: 600; color: #fff; letter-spacing: 0.08em; }'
    + '.header-right .code-label { font-family: "IBM Plex Mono", monospace; font-size: 9px; color: #7a92b4; text-transform: uppercase; letter-spacing: 0.1em; margin-top: 3px; }'
    + '.satellite-wrap { position: relative; width: 100%; background: #1e2d44; border-bottom: 1px solid #243450; line-height: 0; min-height: 180px; }'
    + '.satellite-wrap img.sat-img { width: 100%; display: block; }'
    + '.sat-img-loading { display: flex; align-items: center; justify-content: center; min-height: 220px; color: #7a92b4; font-family: "IBM Plex Mono", monospace; font-size: 12px; flex-direction: column; gap: 10px; }'
    + '.sat-img-error { display: none; align-items: center; justify-content: center; min-height: 180px; background: #1a2a3a; color: #e85d3a; font-family: "IBM Plex Mono", monospace; font-size: 11px; flex-direction: column; gap: 8px; padding: 20px; text-align: center; }'
    + '.sat-label { position: absolute; top: 10px; left: 12px; background: rgba(12,18,32,0.82); font-family: "IBM Plex Mono", monospace; font-size: 10px; color: #f0a500; padding: 3px 8px; border-radius: 4px; border: 1px solid rgba(240,165,0,0.4); letter-spacing: 0.08em; }'
    + '.sat-coords { position: absolute; bottom: 10px; right: 12px; background: rgba(12,18,32,0.82); font-family: "IBM Plex Mono", monospace; font-size: 9px; color: #e8eef6; padding: 3px 8px; border-radius: 4px; }'
    + '.info-row { display: flex; align-items: stretch; border-top: 1px solid #243450; }'
    + '.info-fields { flex: 1; padding: 18px 24px; display: grid; grid-template-columns: 1fr 1fr; gap: 10px 24px; }'
    + '.field-label { font-family: "IBM Plex Mono", monospace; font-size: 9px; text-transform: uppercase; letter-spacing: 0.12em; color: #7a92b4; margin-bottom: 3px; }'
    + '.field-value { font-family: "IBM Plex Mono", monospace; font-size: 13px; color: #e8eef6; font-weight: 600; word-break: break-all; }'
    + '.field-value.highlight { color: #f0a500; font-size: 15px; }'
    + '.field-value.seafoam { color: #3abf9e; }'
    + '.qr-panel { padding: 14px 20px; border-left: 1px solid #243450; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 6px; min-width: 130px; }'
    + '.qr-panel img { width: 100px; height: 100px; border: 3px solid #f0a500; border-radius: 6px; padding: 4px; background: #fff; display: block; }'
    + '.qr-label { font-family: "IBM Plex Mono", monospace; font-size: 8px; color: #7a92b4; text-transform: uppercase; letter-spacing: 0.1em; text-align: center; }'
    + '.layout-footer { background: #0c1220; border-top: 1px solid #243450; padding: 8px 24px; display: flex; justify-content: space-between; align-items: center; }'
    + '.layout-footer span { font-family: "IBM Plex Mono", monospace; font-size: 9px; color: #7a92b4; letter-spacing: 0.08em; }'
    // Watermark
    + '.watermark { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center; pointer-events: none; text-align: center; opacity: 0.55; z-index: 9999; transform: rotate(-30deg); }'
    + '.wm-text { font-family: "Syne", sans-serif; font-size: 80px; font-weight: 800; color: #ff0000; white-space: nowrap; letter-spacing: 0.06em; text-shadow: 0 0 20px rgba(255,0,0,0.9), 2px 2px 0 #000, -2px -2px 0 #000, 2px -2px 0 #000, -2px 2px 0 #000; -webkit-text-stroke: 2px #000; }'
    + '.wm-sub { font-family: "IBM Plex Mono", monospace; font-size: 34px; font-weight: 700; color: #f0a500; white-space: nowrap; letter-spacing: 0.14em; text-shadow: 0 0 16px rgba(240,165,0,0.9), 2px 2px 0 #000, -2px -2px 0 #000; -webkit-text-stroke: 1px #000; }'
    // Pay banner
    + '.pay-banner { max-width: 860px; margin: 16px auto 0; background: linear-gradient(135deg,#1a2a1a,#1e2d44); border: 2px solid #3abf9e; border-radius: 12px; padding: 20px 24px; display: flex; align-items: center; justify-content: space-between; gap: 16px; flex-wrap: wrap; }'
    + '.pay-banner h3 { font-family: "Syne", sans-serif; font-size: 16px; font-weight: 800; color: #3abf9e; margin-bottom: 4px; }'
    + '.pay-banner p { font-family: "IBM Plex Mono", monospace; font-size: 11px; color: #7a92b4; }'
    + '.pay-btn { background: #3abf9e; color: #0c1220; font-family: "Syne", sans-serif; font-size: 15px; font-weight: 800; border: none; border-radius: 8px; padding: 12px 28px; cursor: pointer; letter-spacing: 0.04em; white-space: nowrap; }'
    + '.pay-btn:hover { background: #4dd4b2; }'
    + '.pay-btn:disabled { opacity: 0.5; cursor: not-allowed; }'
    + '.waiting-msg { display: none; max-width: 860px; margin: 12px auto 0; background: rgba(240,165,0,0.1); border: 1px solid rgba(240,165,0,0.4); border-radius: 8px; padding: 12px 20px; font-family: "IBM Plex Mono", monospace; font-size: 12px; color: #f0a500; text-align: center; letter-spacing: 0.06em; }'
    + '.unlocked-banner { display: none; max-width: 860px; margin: 16px auto 0; background: linear-gradient(135deg,#1a3a2a,#1e2d44); border: 2px solid #3abf9e; border-radius: 12px; padding: 20px 24px; text-align: center; }'
    + '.unlocked-banner h3 { font-family: "Syne", sans-serif; font-size: 18px; font-weight: 800; color: #3abf9e; margin-bottom: 6px; }'
    + '.unlocked-banner p { font-family: "IBM Plex Mono", monospace; font-size: 11px; color: #7a92b4; margin-bottom: 14px; }'
    + '.print-btn { display: inline-block; padding: 12px 36px; background: #f0a500; color: #0c1220; font-family: "Syne", sans-serif; font-size: 15px; font-weight: 800; border: none; border-radius: 8px; cursor: pointer; letter-spacing: 0.04em; }'
    + '.print-btn:hover { background: #ffbe33; }'
    + '@keyframes spin { to { transform: rotate(360deg); } }'
    + '@media print { body { background: #fff; padding: 0; } .layout-card { border-color: #c87700; } .no-print { display: none !important; } .watermark { display: none !important; } }'
    + '</style></head><body>'

    // Watermark
    + '<div class="watermark" id="watermark"><div class="wm-text">PREVIEW ONLY</div><div class="wm-sub">PAYMENT REQUIRED</div></div>'

    // Layout card
    + '<div class="layout-card">'
    + '<div class="layout-header">'
    + '<div class="header-left"><div class="logo">DAC</div><div class="logo-sub">Digital Address Codes</div></div>'
    + '<div class="header-right"><div class="code">' + code + '</div><div class="code-label">DAC Address Code</div></div>'
    + '</div>'

    // Satellite
    + '<div class="satellite-wrap">'
    + '<div class="sat-img-loading" id="sat-loading"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#f0a500" stroke-width="1.5" style="animation:spin 1.2s linear infinite"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg>Loading satellite image\u2026</div>'
    + '<div class="sat-img-error" id="sat-error">\u26a0 Satellite image could not be loaded.</div>'
    + '<img class="sat-img" src="' + staticMapUrl + '" style="display:none;" onload="document.getElementById(\'sat-loading\').style.display=\'none\';this.style.display=\'block\';" onerror="document.getElementById(\'sat-loading\').style.display=\'none\';document.getElementById(\'sat-error\').style.display=\'flex\';this.style.display=\'none\';"/>'
    + '<div class="sat-label">\uD83D\uDCE1 SATELLITE VIEW \u00B7 ZOOM ' + zoom + '</div>'
    + '<div class="sat-coords">' + lat + ', ' + lng + '</div>'
    + '</div>'

    // Info row
    + '<div class="info-row"><div class="info-fields">'
    + '<div class="field"><div class="field-label">DAC Address Code</div><div class="field-value highlight">' + code + '</div></div>'
    + '<div class="field"><div class="field-label">Parish / Region</div><div class="field-value seafoam">' + regionName + '</div></div>'
    + '<div class="field"><div class="field-label">Latitude</div><div class="field-value">' + lat + '</div></div>'
    + '<div class="field"><div class="field-label">Longitude</div><div class="field-value">' + lng + '</div></div>'
    + '<div class="field"><div class="field-label">Generated</div><div class="field-value">' + dateStr + '</div></div>'
    + '<div class="field"><div class="field-label">System</div><div class="field-value">DAC Grid v3.0</div></div>'
    + '</div>'
    + '<div class="qr-panel">' + qrHtml + '<div class="qr-label">Scan for<br>directions</div></div>'
    + '</div>'
    ...
    + '<span>' + code + ' \u00B7 ' + parseFloat(lat).toFixed(5) + ', ' + parseFloat(lng).toFixed(5) + '</span>'
    + '</div></div>'

    // Payment banner
    + '<div class="pay-banner no-print" id="pay-banner">'
    + '<div><h3>\uD83D\uDD12 Unlock Clean Print</h3><p>Remove watermark \u00B7 Enable printing \u00B7 Save as PDF</p></div>'
    + '<button class="pay-btn" id="pay-btn" onclick="startPayment()">\uD83D\uDCB3 Pay to Unlock \u2014 $3</button>'
    + '</div>'

    + '<div class="waiting-msg no-print" id="waiting-msg">\u23F3 Waiting for payment\u2026 this page will unlock automatically once payment is confirmed.</div>'

    // Unlocked banner
    + '<div class="unlocked-banner no-print" id="unlocked-banner">'
    + '<h3>\u2705 Payment Confirmed \u2014 Layout Unlocked!</h3>'
    + '<p>Watermark removed \u00B7 You can now print or save as PDF</p>'
    + '<button class="print-btn" onclick="window.print()">\uD83D\uDDB8 Print / Save as PDF</button>'
    + '</div>'

    + '<script>'
    + 'var CODE="' + code + '";'
    + 'var PAID_KEY="dac_paid_"+CODE;'
    + 'var STRIPE_URL="' + stripeUrl + '";'
    + 'var pollInterval=null;'
    + 'var stripeTab=null;'
    // Check immediately if already paid
    + 'if(localStorage.getItem(PAID_KEY)==="1"){unlockLayout();}'
    + 'function startPayment(){'
    +   'localStorage.setItem("dac_pending_payment_code",CODE);'
    +   'stripeTab=window.open(STRIPE_URL,"_blank");'
    +   'document.getElementById("pay-btn").disabled=true;'
    +   'document.getElementById("pay-btn").textContent="\u23F3 Processing\u2026";'
    +   'document.getElementById("waiting-msg").style.display="block";'
    +   'pollInterval=setInterval(function(){'
    +     'if(localStorage.getItem(PAID_KEY)==="1"){clearInterval(pollInterval);unlockLayout();}'
    +   '},2000);'
    ...
    +   'document.getElementById("unlocked-banner").style.display="block";'
    +   'if(stripeTab&&!stripeTab.closed){try{stripeTab.close();}catch(e){}}'
    + '}'
    + '<\/script>'
    + '</body></html>');
  win.document.close();
...
document.addEventListener('click', (e) => {
  if (e.target.id === 'btn-paid-layout' || e.target.closest('#btn-paid-layout')) {
    generatePropertyLayout();
  }
});
