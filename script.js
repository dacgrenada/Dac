/**
 * GrenLoc v3.0 – Global Digital Location Code System
 * ─────────────────────────────────────────────────────
...
function loadRegistry() {
  try { return JSON.parse(localStorage.getItem('grenloc_registry') || '{}'); }
  catch { return {}; }
}

function saveRegistry(reg) {
  try { localStorage.setItem('grenloc_registry', JSON.stringify(reg)); } catch {}
}

function loadReverseIndex() {
  try { return JSON.parse(localStorage.getItem('grenloc_reverse') || '{}'); }
  catch { return {}; }
}

function saveReverseIndex(idx) {
  try { localStorage.setItem('grenloc_reverse', JSON.stringify(idx)); } catch {}
}
...
function initMap() {
  ...
  console.log('%cGrenLoc v3.0 — Global CC-PPP-XXXXXX Format', 'color:#f0a500;font-weight:bold;font-size:14px;');
}
...
function drawParishBoxes() {
  ...
  PARISHES.forEach((parish, i) => {
    ...
    rect.addListener('mouseover', (e) => {
      infoWindow.setContent(`<div style="font-family:'IBM Plex Mono',monospace;font-size:12px;color:#0c1220;padding:2px 4px;"><strong>${parish.code}</strong><br>${parish.name}</div>`);
...
function updateUI(data) {
  ...
  ['btn-copy','btn-maps','btn-download','btn-print','btn-share','btn-layout'].forEach(id => {
  ...
  document.getElementById('btn-maps').onclick = () => window.open(data.mapsUrl, '_blank');
...
document.addEventListener('click', (e) => {
  ...
  link.href = url;
  link.download = (currentCode || 'grenloc') + '-qr.png';
  link.click();
...
document.addEventListener('click', (e) => {
  ...
  win.document.write(`
    <html><head><title>GrenLoc Sticker - ${currentCode}</title>
    <style>
...
document.addEventListener('click', (e) => {
  ...
  if (navigator.share) {
    navigator.share({ title: 'GrenLoc', text, url: currentMapsUrl });
  } else {
...
async function generatePropertyLayout() {
  if (!currentCode || currentLat === null) {
    showToast('Pin a property first');
    return;
  }

  showToast('Generating property layout...');
  ...
  if (img.complete && img.naturalWidth > 0) {
    ctx.drawImage(img, 100, 80, 600, 400);
    ctx.strokeStyle = '#f0a500';
    ctx.lineWidth   = 2;
    ctx.strokeRect(100, 80, 600, 400);
  } else {
    ctx.fillStyle = '#1e2d44';
    ctx.fillRect(100, 80, 600, 400);
    ctx.strokeStyle = '#f0a500';
    ctx.lineWidth   = 2;
    ctx.strokeRect(100, 80, 600, 400);
    ctx.fillStyle   = '#7a92b4';
    ctx.font        = '16px IBM Plex Mono, monospace';
    ctx.textAlign   = 'center';
    ctx.fillText('Satellite map preview', 400, 270);
    ctx.font = '13px IBM Plex Mono, monospace';
    ctx.fillText(`${currentLat.toFixed(6)}, ${currentLng.toFixed(6)}`, 400, 295);
  }
  ...
  ctx.strokeStyle = '#f0a500';
  ctx.lineWidth = 1;
  ctx.beginPath(); ctx.moveTo(0, 76); ctx.lineTo(800, 76); ctx.stroke();

  ctx.fillStyle = '#f0a500';
  ctx.font      = 'bold 26px Syne, sans-serif';
  ctx.textAlign = 'left';
  ctx.fillText('GrenLoc', 24, 46);

  ctx.fillStyle = '#ffffff';
  ctx.font      = 'bold 22px IBM Plex Mono, monospace';
  ctx.textAlign = 'right';
  ctx.fillText(currentCode, 776, 38);

  ctx.fillStyle = '#7a92b4';
  ctx.font      = '12px IBM Plex Mono, monospace';
  ctx.fillText('PROPERTY LAYOUT', 776, 58);

  ctx.fillStyle = '#141e30';
  ctx.fillRect(0, 510, 800, 90);
  ctx.strokeStyle = '#243450';
  ctx.lineWidth = 1;
  ctx.beginPath(); ctx.moveTo(0, 510); ctx.lineTo(800, 510); ctx.stroke();
  ...
  ctx.textAlign = 'left';
  ctx.fillText(`Code:      ${currentCode}`, 24, 533);
  ctx.fillText(`Lat/Lng:   ${currentLat.toFixed(7)}, ${currentLng.toFixed(7)}`, 24, 553);
  ctx.fillText(`Region:    ${regionName}`, 24, 573);
  ctx.fillText(`Generated: ${dateStr}`, 24, 593);

  ctx.textAlign = 'right';
  ctx.fillStyle = '#7a92b4';
  ctx.font      = '11px IBM Plex Mono, monospace';
  ctx.fillText('GrenLoc Grid System v3.0', 776, 533);
  ctx.fillText('grenloc.gnd@gmail.com', 776, 553);
  ctx.fillText('Global Location Code System', 776, 573);
  ...
  if (!IS_PAID) {
    ...
    ctx.textAlign = 'center';
    ctx.fillText('GRENLOC', 0, -18);
    ctx.font      = 'bold 18px IBM Plex Mono, monospace';
    ctx.fillStyle = '#f0a500';
    ctx.fillText('NOT PAID VERSION', 0, 18);
    ctx.restore();
    ...
    ctx.textAlign = 'center';
    ctx.fillText('GRENLOC - NOT PAID VERSION', 0, 0);
    ctx.restore();
  }
  ...
  canvas.toBlob((blob) => {
    ...
    const link = document.createElement('a');
    link.href  = url;
    link.download = `GrenLoc-PropertyLayout-${currentCode}.png`;
    link.click();
    URL.revokeObjectURL(url);
    showToast('Property layout downloaded!');
  }, 'image/png');
}
