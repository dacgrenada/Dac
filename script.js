...
function generatePropertyLayout(isPaid = false) {
  ...
  // Uses window.addEventListener('storage') for instant cross-tab detection
  // + Supabase polling as a reliable fallback.
...
<script>
  ...
  // Clear any stale flags on open
  try { localStorage.removeItem(PAID_KEY); } catch(e) {}
  try { localStorage.removeItem('dac_pending_payment_code'); } catch(e) {}

  // ── INSTANT: storage event fires in THIS window when ANOTHER tab writes localStorage ──
  ...
  function startPayment() {
    ...
    // ── FALLBACK: Poll Supabase every 3s in case storage event is missed ──
    // (e.g. browser privacy settings block cross-tab localStorage events)
    supaInterval = setInterval(async function() {
      ...
      try {
        ...
        const { data, error } = await client
          ...
          .single();
        if (!error && data && data.paid_layout === true) {
          triggerUnlock();
        }
